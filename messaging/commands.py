...
async def handle_stop_command(
    ...
    """Handle /stop: reply-scoped cancels one task, bare /stop cancels all in-flight."""
...
async def _delete_message_ids(
    ...
    """Delete message IDs best-effort; numeric IDs sorted descending, errors swallowed."""
    if not msg_ids:
        return
...
async def _handle_clear_branch(
    ...
    """Cancel tasks, delete platform messages, then remove branch from tree (order is load-bearing)."""
...
async def handle_clear_command(
    ...
    """Handle /clear: reply-scoped removes a branch; bare /clear resets the entire session."""
